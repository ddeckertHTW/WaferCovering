...
## Organization
This subfolder is organized in 3 layers. In the first layer, the output files are separated by the problem type (default = "100 Percent", contamination = "diagonal error", test structures = "mod 4"). In the next layer, the instances are categorized by wafer diameter, and in the last layer, the individual files are separated by probe card shape.
